...
/**
 * Реализует базовые возможности хитбокса<br>
 * <br>
 * Доступно:<br>
 * <br>
 * Point position - центр хитбокса<br>
 * float angle - угол поворота хитбокса<br>
 * Point[] vertex - массив вершин хитбокса (относительно (0;0)). Для получения
 * актуальных вершин вызывайте getVertex() - возвращает Point[]<br>
...
//				radius = this.position.getDistance(vertex[i]);
	...
	public Point[] getVertex() {
		for (int i = 0; i < vertex.length; i++) {
			vertexBuffer[i].paste(vertex[i]).rotate(angle).add(position);
		}
		return vertexBuffer;
	}
	@Override
